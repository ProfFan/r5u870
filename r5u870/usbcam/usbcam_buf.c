				   ...
				   enum v4l2_field field)
{
	...
	if (framep->vbb.state == STATE_NEEDS_INIT) {
		...
		usbcam_dbg(udp, VIDEOBUF, "%s, field=%d, vbb={w: %u, h: %u, memory: %d, size: %lu, bsize: %u, baddr: %lu}\n",
			__func__, field, framep->vbb.width, framep->vbb.height, framep->vbb.memory, framep->vbb.size, framep->vbb.bsize, framep->vbb.baddr);

		usbcam_dbg(udp, VIDEOBUF,
			   "preparing frame %p/%d/%p", vq, framep->vbb.i, framep);
		usbcam_dbg(udp, VIDEOBUF,
			   "vbb/vb %p/%p", &framep->vbb, vb);
